...
describe("createWorkflowContext", () => {
  ...
  test("allows autorun to override the agent cwd while keeping artifacts under control cwd", () => {
    const context = createWorkflowContext(baseOptions, {
      agentCwd: "/repo/.roark/worktrees/issue-10",
    });
    expect(context.controlCwd).toBe(path.resolve("/repo"));
    expect(context.agentCwd).toBe(
      path.resolve("/repo/.roark/worktrees/issue-10"),
    );
    expect(context.runDir).toBe(path.resolve("/repo", ".roark/runs/issue/10"));
    expect(context.runDirRelative).toBe(
      path.join(".roark/runs", "issue", "10"),
  ...
  test("nests runDir under attempts/<n> when attempt is supplied", () => {
    ...
    expect(context.runDir).toBe(
      path.resolve("/repo", ".roark/runs/issue/10/attempts/2"),
    );
    expect(context.runDirRelative).toBe(
      path.join(".roark/runs", "issue", "10", "attempts", "2"),
    );
  ...
  test("maps issue creation results artifact", () => {
    const context = createWorkflowContext(baseOptions);
    expect(artifactRelativePath(context, "issueCreationResults")).toBe(
      path.join(".roark/runs", "issue", "10", "issue-creation-results.json"),
    );
  });
  ...
  test("maps agent artifact paths from split autorun worktrees", () => {
    ...
    expect(artifactAgentPath(context, "issue")).toBe(
      path.join("..", "..", "runs", "issue", "10", "issue.md"),
    );
  });
});
