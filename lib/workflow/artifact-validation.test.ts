...
describe("validateAgentArtifact", () => {
  test("rejects an empty review artifact", () => {
    const result = Effect.runSync(validateAgentArtifact(reviewBRef(0), ""));
  ...
  test("accepts only structured review artifacts", () => {
    expect(
      Effect.runSync(
        validateAgentArtifact(reviewARef(0), JSON.stringify(reviewResult())),
      ),
    ).toEqual({ ok: true });
    expect(
      Effect.runSync(validateAgentArtifact(reviewBRef(2), "Looks fine.")).ok,
    ).toBe(false);
  });
    ...
    expect(
      Effect.runSync(
        validateAgentArtifact(
          "implementationPlan",
          JSON.stringify(implementationPlanResult()),
        ),
      ),
    ).toEqual({ ok: true });
  ...
  test("accepts only structured implementation, refinement, and fix reports", () => {
    ...
    expect(
      Effect.runSync(validateAgentArtifact("implementationLog", content)),
    ).toEqual({
      ok: true,
    });
    expect(
      Effect.runSync(validateAgentArtifact(refinementLogRef(0), content)),
    ).toEqual({
      ok: true,
    });
    expect(
      Effect.runSync(validateAgentArtifact(fixLogRef(1), content)),
    ).toEqual({ ok: true });
    expect(
      Effect.runSync(
        validateAgentArtifact("implementationLog", "# Implementation Log\n"),
      ).ok,
    ).toBe(false);
  });
});
