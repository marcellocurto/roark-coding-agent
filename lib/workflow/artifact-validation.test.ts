...
describe("validateAgentArtifact", () => {
  ...
  test("accepts only structured implementation plans", () => {
    ...
    const result = validateAgentArtifact(
      "implementationPlan",
  ...
  test("accepts only structured implementation, refinement, and fix reports", () => {
    ...
    expect(validateAgentArtifact("implementationLog", content)).toEqual({
      ok: true,
    });
    expect(validateAgentArtifact(refinementLogRef(0), content)).toEqual({
      ok: true,
    });
