...
import * as nativeIssueCuration from "./issue-curation.ts";
...
describe("buildIssueCurationPlan", () => {
  test("no reviewer findings produces an empty plan without crashing", async () => {
    ...
    await runApplicationPromise(
      writeArtifact(context, reviewARef(0), reviewWithLedger("None")),
    );
    await runApplicationPromise(
  ...
  test("one actionable follow-up produces a follow-up issue item", async () => {
      ...
      writeArtifact(
        ...
        reviewWithLedger(
          finding("F1", "follow-up", {
            ...
            evidence:
              "src/retry.ts:17 demonstrates the missing user-facing description.",
            impact:
              "Future users cannot understand how retry exhaustion is reported.",
            handling:
              "Add focused documentation for retry exhaustion behavior.",
          }),
        ),
      ),
    );
    ...
    expect(
      item?.proposedBody.startsWith(
        "## Summary\n\nDocument retry edge case for users.",
      ),
    ).toBe(true);
    ...
    expect(item?.proposedBody).toContain(
      "Source issue: #42 Source issue title",
    );
    expect(item?.proposedBody).not.toContain("## Source");
    expect(item?.proposedBody).not.toContain(
  ...
  test("numbered autorun review artifacts are used when present", async () => {
    ...
    await runApplicationPromise(
      writeArtifact(
        ...
        reviewARef(0),
        reviewWithLedger(
          finding("N1", "follow-up", {
            title: "Document autorun review artifact handling",
            evidence:
              "lib/workflow/issue-curation.ts:116 reads the review artifact selected for curation.",
            impact:
              "Reviewer findings from normal autorun attempts are promoted into follow-up issues.",
            handling:
              "Use numbered review artifacts when curating autorun findings.",
          }),
        ),
      ),
    );
    ...
    expect(plan.warnings).not.toContain(
      "review-a-0.json is missing; treating Review Agent A findings as empty.",
    );
  });
  test("ignores unnumbered review JSON files", async () => {
    ...
    await Bun.write(
      ...
      reviewWithLedger(finding("OLD", "follow-up")),
    );
    await Bun.write(
    ...
    expect(plan.run.artifactPaths).not.toContain(
      ".roark/runs/issue/42/attempts/2/review-a.json",
    );
    expect(plan.run.artifactPaths).not.toContain(
      ".roark/runs/issue/42/attempts/2/review-b.json",
    );
  });
  test("one actionable external-blocker produces a blocking issue item", async () => {
    ...
    await runApplicationPromise(
      writeArtifact(
        ...
        reviewARef(0),
        reviewWithLedger(
          finding("B1", "external-blocker", {
            title: "Missing prerequisite API token fixture",
            evidence:
              "tests/fixtures/token.json:1 is required but absent from the repository.",
            impact:
              "The current issue cannot be validated until the prerequisite fixture exists.",
            handling:
              "Create a separate prerequisite issue to define and provide the fixture.",
          }),
        ),
      ),
    );
    ...
    expect(item?.proposedLabels).toEqual([
      "needs-triage",
      "review:external-blocker",
    ]);
    expect(item?.whyBlockingOrNonBlocking).toContain(
      "prerequisite or external work",
    );
  });
  test("approval-blocking review limitations produce external-blocker issue items", async () => {
            ...
            limitations: [
              {
                ...
                description:
                  "Generated migration output could not be inspected.",
                blocksApproval: true,
              },
            ],
          }),
    ...
    expect(plan.issuesToCreate[0]?.sourceFindingIds).toEqual([
      "review-a:limitation:generated-migration-unavailable",
    ]);
  });
  test("suggestion findings become issues while must-fix-current findings are rejected", async () => {
    ...
    expect(plan.issuesToCreate[0]?.proposedLabels).toEqual([
      "needs-triage",
      "review:suggestion",
    ]);
    expect(
      plan.rejectedCandidates.map((candidate) => candidate.sourceFindingIds[0]),
    ).toEqual(["review-a:m1"]);
    expect(plan.rejectedCandidates[0]?.reason).toContain(
      "current issue/fix pass",
    );
  ...
  test("missing evidence causes rejection", async () => {
    ...
    await runApplicationPromise(
      writeArtifact(
        ...
        reviewARef(0),
        reviewWithLedger(
          finding("F1", "follow-up", { evidence: "unspecified" }),
        ),
      ),
    );
    await runApplicationPromise(
  ...
  test("vague or speculative candidates are rejected", async () => {
      ...
      writeArtifact(
        ...
        reviewWithLedger(
          finding("F1", "follow-up", {
            ...
            handling:
              "Investigate the behavior and decide whether anything should change.",
          }),
        ),
      ),
    );
    ...
    expect(plan.issuesToCreate).toEqual([]);
    expect(plan.rejectedCandidates[0]?.reason).toBe(
      "vague or speculative candidate",
    );
  });
      ...
      writeArtifact(
        ...
        reviewWithLedger(
          finding("F1", "follow-up", {
            ...
            evidence:
              "src/retry.ts:17 does not describe retry exhaustion behavior.",
  ...
  test("metadata supplies stable source issue and run context", async () => {
    ...
    await runApplicationPromise(
      writeArtifact(
        context,
        reviewARef(0),
        reviewWithLedger(finding("F1", "follow-up")),
      ),
    );
    await runApplicationPromise(
      writeArtifact(context, reviewBRef(0), reviewWithLedger("None")),
    ...
    expect(item?.runContext.artifactPaths).toContain(
      ".roark/runs/issue/42/attempts/2/implementation-log.json",
    );
  });
  test("PR context is preserved in plan and generated issue bodies", async () => {
    ...
    await runApplicationPromise(
      writeArtifact(
        context,
        reviewARef(0),
        reviewWithLedger(finding("F1", "suggestion")),
      ),
    );
    await runApplicationPromise(
      writeArtifact(context, reviewBRef(0), reviewWithLedger("None")),
    ...
    expect(item?.runContext.prUrl).toBe(
      "https://github.com/owner/repo/pull/99",
    );
    expect(item?.proposedBody).toContain(
      "Related PR: https://github.com/owner/repo/pull/99",
    );
  ...
  test("available artifact paths include catalog static refs and numbered review refs", async () => {
    const context = await tempContext();
    await runApplicationPromise(writeArtifact(context, "metadata", "{}\n"));
    await runApplicationPromise(
...
describe("issue curation phase", () => {
  test("manual curate-issues without --attempt gives guidance when attempt artifacts exist", async () => {
    ...
    await mkdir(path.join(dir, ".roark/runs/issue/42/attempts/3"), {
      recursive: true,
    });
    try {
      await runApplicationPromise(
        nativePhases.runSinglePhase(context, "curate-issues"),
      );
  ...
  test("runSinglePhase writes issue-curation-plan.json without using an agent", async () => {
    ...
    expect(
      await runApplicationPromise(artifactExists(context, "issueCurationPlan")),
    ).toBe(true);
...
function reviewWithLedger(
  entries: ReviewFinding | ReviewFinding[] | "None",
): string {
  const findings =
    entries === "None" ? [] : Array.isArray(entries) ? entries : [entries];
...
function finding(
  ...
  return reviewFinding(classification, overrides.title ?? `Finding ${_id}`, {
    ...
    evidence:
      typeof overrides.evidence === "string"
        ? [overrides.evidence]
        : (overrides.evidence ?? ["src/example.ts:1 shows concrete behavior."]),
    currentIssueImpact:
      overrides.impact ?? "Future users encounter a concrete gap.",
    recommendedHandling:
      overrides.handling ?? "Create a focused follow-up issue for this gap.",
    ...(overrides.suggestedIssueTitle
      ? { suggestedIssueTitle: overrides.suggestedIssueTitle }
      : {}),
  });
}
