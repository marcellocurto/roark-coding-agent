...
function issueCandidateRejectionReason(
  ...
  if (finding.classification === "must-fix-current")
    return "must-fix-current findings belong to the current issue/fix pass and are not promoted by default";
  if (!finding.evidence.some(hasConcreteContent))
    return "missing concrete evidence";
  if (!hasConcreteContent(finding.currentIssueImpact))
    return "missing current-issue or future-user impact";
  if (
    !hasConcreteContent(finding.title) ||
    !hasConcreteContent(finding.recommendedHandling)
  )
    return "missing actionable title or recommended handling";
  if (hasVagueOrSpeculativeLanguage(finding))
...
function hasConcreteContent(value: string): boolean {
  ...
  if (
    /^(unspecified|none|n\/a|not applicable|unknown|unclear|tbd|todo)$/.test(
      normalized,
    )
  )
    return false;
...
function hasVagueOrSpeculativeLanguage(
  ...
  const value = [
    ...
    .join("\n")
    .toLowerCase();
  return /\b(maybe|perhaps|possibly|speculative|unclear|unknown|not sure|might|seems like|appears to)\b/.test(
    value,
  );
}
...
function compareFindingsForGrouping(
  ...
  const titleComparison = normalizeTitle(left).localeCompare(
...
function buildIssuePlanItems(input: {
  ...
  return input.groups.map((group, index) => {
    ...
    const sourceClassifications = unique(
      group.map((finding) => finding.classification),
    );
    ...
    const handling = unique(
      group.map((finding) => finding.recommendedHandling),
    );
    const proposedTitle =
      representative.suggestedIssueTitle ?? representative.title;
    ...
    const item: IssuePlanItem = {
      ...
      severitySummary: summarizeField(
        "severity",
        group.map((finding) =>
          "severity" in finding ? finding.severity : "not applicable",
        ),
      ),
      confidenceSummary: summarizeField(
        "confidence",
        group.map((finding) =>
          "confidence" in finding ? finding.confidence : "not applicable",
        ),
      ),
      evidence,
      impact: impacts.join("\n\n"),
      ...
      proposedLabels: unique([
        ...reviewerIssueTriageLabels,
        reviewerIssueLabelForClassification(input.classification),
      ]),
    };
...
function classificationExplanation(
  ...
  if (classification === "external-blocker")
    return "A reviewer found prerequisite or external work that may explain why the source issue could not proceed normally.";
...
function buildProposedIssueBody(input: { item: IssuePlanItem }): string {
  ...
  const attempt =
    input.item.runContext.attempt === undefined
      ? "not specified"
      : String(input.item.runContext.attempt);
  const prLine = input.item.runContext.prUrl
    ? `- Related PR: ${input.item.runContext.prUrl}\n`
    : "";
  const triage = triageRecommendation(input.item.classification);
  const nonGoals =
    input.item.classification === "follow-up" ||
    input.item.classification === "suggestion"
  ...
  const evidence = input.item.evidence
...
function triageRecommendation(classification: IssuePlanClassification): {
  ...
  if (classification === "external-blocker") {
    return {
      priority:
        "High if it is still blocking the source issue; otherwise triage manually",
...
async function loadSourceIssueContext(
  ...
  const fallback: IssueCurationPlan["sourceIssue"] = {
    ...
    ...(context.repo
      ? {
          url: `https://github.com/${context.repo}/issues/${context.issueNumber}`,
        }
      : {}),
  };
  ...
  if (metadata) {
    try {
      ...
      const number = Number.isInteger(parsedNumber)
        ? parsedNumber
        : fallback.number;
      const title = parsed.issue?.title ?? fallback.title;
      const url =
        parsed.issue?.html_url ??
        parsed.issue?.htmlUrl ??
        parsed.issue?.url ??
        (parsed.repo
...
function parseIssueArtifact(
  ...
  const number =
    numberMatch?.[1] !== undefined
      ...
      : markdownTitleMatch?.[1] !== undefined
        ? Number(markdownTitleMatch[1])
        : fallback.number;
  const title =
    titleMatch?.[1] !== undefined
      ? decodeXmlText(titleMatch[1].trim())
      : (markdownTitleMatch?.[2]?.trim() ?? fallback.title);
