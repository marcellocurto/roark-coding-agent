...
export type PullRequestFeedback = {
  ...
  excludedRoarkSummaryCommentIds: Array<string | number>;
  fetchedAt: string;
};
...
export async function fetchPullRequestFeedback(options: { cwd: string; repo?: string; prNumber: number }): Promise<PullRequestFeedback> {
...
export async function resolvePullRequestRepo(options: { cwd: string; repo?: string }): Promise<string> {
...
export function parsePullRequestFeedback(raw: string, input: { repo: string; prNumber: number }): PullRequestFeedback {
  ...
  const comments = connectionNodes(pullRequest.comments).map(normalizePullRequestComment).filter((comment) => comment.body !== undefined) as PullRequestComment[];
  ...
  const excludedRoarkSummaryCommentIds: Array<string | number> = [];
