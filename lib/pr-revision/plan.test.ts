...
describe("structured PR revision plans", () => {
  ...
  test("preserves freely named planning context without changing status routing", () => {
        ...
        additionalSections: [
          {
            ...
            items: [
              "The two comments describe the same already-addressed behavior from different call sites.",
            ],
          },
        ],
      }),
    ...
    expect(formatRevisionPlanMarkdown(result)).toContain(
      "## Interaction between comments",
    );
  });
  ...
  test("rejects invented source identities and item ids not derived from a source", () => {
    ...
    expect(() =>
      Effect.runSync(validateRevisionPlanResult(base, new Set(["comment:1"]))),
    ...
    ).toThrow("must derive from one of its source ids");
  });
});
