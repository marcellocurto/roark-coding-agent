...
describe("autorun verification", () => {
  ...
  test("default verification terminates the process tree when a command exceeds its timeout", async () => {
    ...
    const result = await runVerificationPromise({
  ...
  test("writes complete verification companions through the workflow artifact catalog", async () => {
    ...
    try {
      ...
      expect(await artifactExists(context, "verificationFull")).toBe(true);
      expect(
        await artifactExists(context, verificationBeforeFixFullRef(2)),
      ).toBe(true);
      expect(await readArtifact(context, "verificationFull")).toContain(
        "complete stdout",
      );
    } finally {
