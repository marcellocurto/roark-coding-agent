...
describe("redactLocalPaths", () => {
  test("redacts obvious POSIX local paths", () => {
    expect(
      redactLocalPaths(
        "see /Users/alice/repo and /home/alice/repo and /tmp/roark/repo",
      ),
    ).toBe(
  ...
  test("redacts delimiter-adjacent POSIX paths", () => {
    expect(
      redactLocalPaths(
        "path:/Users/alice/repo [ /home/alice/repo ] [/tmp/roark/repo]",
      ),
    ).toBe(
  ...
  test("redacts Windows paths", () => {
    expect(
      redactLocalPaths("C:\\Users\\alice\\repo and C:/Users/alice/repo"),
    ).toBe("[local path redacted] and [local path redacted]");
  });

  test("redacts POSIX file URIs", () => {
    expect(
      redactLocalPaths(
        "at file:///Users/alice/repo/src/index.ts:10:2 then https://github.com/owner/repo",
      ),
    ).toBe("at [local path redacted] then https://github.com/owner/repo");
  });

  test("redacts full paths containing spaces", () => {
    expect(
      redactLocalPaths(
        "at /Users/John Doe/repo/src/index.ts and C:\\Users\\John Doe\\repo\\file.ts",
      ),
    ).toBe("at [local path redacted] and [local path redacted]");
    expect(redactLocalPaths("cwd /Users/alice/My Repo failed")).toBe(
      "cwd [local path redacted] failed",
    );
  });

  test("preserves web URLs, site paths, and repo-relative paths", () => {
    expect(redactLocalPaths("https://github.com/owner/repo/issues/1")).toBe(
      "https://github.com/owner/repo/issues/1",
    );
    expect(
      redactLocalPaths("serve /llms.txt and /pricing from public/llms.txt"),
    ).toBe("serve /llms.txt and /pricing from public/llms.txt");
  });
});
...
describe("public markdown sanitization", () => {
  test("redacts obvious token assignments and authorization headers", () => {
    expect(
      redactSecrets(
        "GITHUB_TOKEN=abc API_KEY: xyz Authorization: Bearer secret",
      ),
    ).toBe(
