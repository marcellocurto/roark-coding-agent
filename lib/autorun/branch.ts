...
export function assertSafeWorkBranch(options: {
  ...
  if (branchName === baseBranch)
    throw new Error(
      `Autorun work branch cannot be the base branch '${baseBranch}'.`,
    );
  if (branchName === defaultAutorunBaseBranch)
    throw new Error(
      `Autorun work branch cannot be '${defaultAutorunBaseBranch}'.`,
    );
}
...
export async function ensureRoarkWorktreesIgnored(
  controlCwd: string,
): Promise<void> {
...
export async function ensureIssueWorktree(
  options: { controlCwd: string; plan: AutorunBranchPlan },
  application?: ApplicationExecution,
): Promise<string> {
  if (!application)
    return runApplicationPromise(
      fromLegacyPromise((application) =>
        ensureIssueWorktree(options, application),
      ),
      application,
    );
  ...
  if (existsSync(agentCwd)) {
    ...
    await assertWorktreeOnBranch(
      { agentCwd, branchName: options.plan.branchName },
      application,
    );
  ...
  if (
    await gitBranchExists(
      { cwd: options.controlCwd, branchName: options.plan.branchName },
      application,
    )
  ) {
    ...
    await runProcessOrThrowPromise(
      ["git", "fetch", "origin"],
      { cwd: options.controlCwd, label: "git fetch origin" },
      application,
    );
    await runProcessOrThrowPromise(
      [
...
export async function checkoutIssueBranch(
  options: { cwd: string; plan: AutorunBranchPlan },
  application?: ApplicationExecution,
): Promise<void> {
  if (!application)
    return runApplicationPromise(
      fromLegacyPromise((application) =>
        checkoutIssueBranch(options, application),
      ),
      application,
    );

  await ensureIssueWorktree(
    { controlCwd: options.cwd, plan: options.plan },
    application,
  );
}

export async function checkoutExistingIssueBranch(
  options: { cwd: string; plan: AutorunBranchPlan; worktreePath?: string },
  application?: ApplicationExecution,
): Promise<string> {
  if (!application)
    return runApplicationPromise(
      fromLegacyPromise((application) =>
        checkoutExistingIssueBranch(options, application),
      ),
      application,
    );
  ...
  await runProcessOrThrowPromise(
    ["git", "worktree", "prune"],
    { cwd: options.cwd, label: "git worktree prune" },
    application,
  );

  if (
    await gitBranchExists(
      { cwd: options.cwd, branchName: options.plan.branchName },
      application,
    )
  ) {
    await runProcessOrThrowPromise(
...
async function assertWorktreeOnBranch(
  options: { agentCwd: string; branchName: string },
  application?: ApplicationExecution,
): Promise<void> {
  const currentBranch = (
    await runProcessOrThrowPromise(
...
async function fetchOriginIfAvailable(
  cwd: string,
  application?: ApplicationExecution,
): Promise<void> {
