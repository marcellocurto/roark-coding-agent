...
describe("planContinuation", () => {
    ...
    expect(steps).toEqual([
      {
        ...
        reason:
          'triage verdict is "needs-human-decision"; readiness records the stop',
      },
      {
        type: "noop",
        reason: "terminal triage outcome; no plan/implementation/publish gate",
      },
    ]);
  });
  ...
  test("does not plan implementation for a valid refined plan that is not ready", async () => {
    ...
    expect(steps).toEqual([
      {
        type: "write-readiness",
        reason: "implementation plan is not ready; readiness records the stop",
      },
      {
        type: "noop",
        reason: "terminal planning outcome; no implementation/publish gate",
      },
    ]);
  });
