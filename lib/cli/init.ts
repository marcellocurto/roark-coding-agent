...
export async function runInit(
  ...
  if (!application)
    return runApplicationPromise(
      fromLegacyPromise((application) => runInit(options, deps, application)),
      application,
    );
  ...
  const rawRepo =
    options.repo ??
    (await inferRepoFromOrigin(options.cwd, runner, application));
  if (!rawRepo) {
    throw new Error(
  ...
  const repo = options.repo
    ? rawRepo
    : await canonicalizeGitHubRepo(rawRepo, runner, application);
  ...
  const conflicts = managedFiles.filter((relativePath) =>
    existsSync(path.join(options.cwd, relativePath)),
  );
...
async function canonicalizeGitHubRepo(
  repo: string,
...
async function inferSetupHook(workspace: string): Promise<string | undefined> {
  ...
  if (existsSync(path.join(workspace, "pnpm-lock.yaml")))
  ...
  if (existsSync(path.join(workspace, "yarn.lock"))) {
    try {
      ...
      return lockfile.startsWith("# yarn lockfile v1")
...
async function ensureRoarkDirectory(workspace: string): Promise<void> {
  ...
  if (existsSync(roarkDir)) {
    ...
    if (!existing.isDirectory())
      throw new Error(`${roarkDir} exists but is not a directory.`);
    return;
  }
...
function assertOwnerRepo(repo: string): void {
  ...
  throw new Error(
    `GitHub repository must be in owner/repo form. Got '${repo}'.`,
  );
}
