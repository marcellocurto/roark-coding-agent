...
import { executeProcess, ProcessExecutionError, ProcessExitError, runProcess, runProcessOrThrow } from "./process.ts";
...
describe("Effect process execution", () => {
  ...
  test("closes absent stdin and retains nonzero exits until explicitly rejected", async () => {
    expect(await runProcess(["sh", "-c", "cat; printf failure >&2; exit 7"])).toEqual({ stdout: "", stderr: "failure", exitCode: 7 });
    expect(runProcessOrThrow(["sh", "-c", "printf failure >&2; exit 7"], { label: "verification" })).rejects.toBeInstanceOf(ProcessExitError);
  });

  test("rejects when the child exits before stdin can be delivered", async () => {
    const result = await runProcess(["sh", "-c", "exit 0"], { input: "x".repeat(10_000_000) }).then(
      () => undefined,
      (error: unknown) => error,
    );
  ...
  test("returns signal exit codes and typed launch errors", async () => {
    expect((await runProcess(["sh", "-c", "kill -TERM $$"])).exitCode).toBe(143);
    expect(runProcess(["roark-command-that-does-not-exist"])).rejects.toBeInstanceOf(ProcessExecutionError);
  });
  ...
  for (const parentExit of [false, true]) {
    test(`interruption through a legacy caller cleans descendants when parent ${parentExit ? "has exited" : "is alive"}`, async () => {
      ...
      let pid: number | undefined;
      const running = Effect.runPromiseExit(fromLegacyPromise(() => runProcess([
        "sh", "-c", `sleep 30 & echo $! > child.pid; ${parentExit ? "exit 0" : "wait"}`,
      ], { cwd })).pipe(Effect.provide(applicationLayer)), { signal: controller.signal });
      try {
        pid = await waitForPid(path.join(cwd, "child.pid"));
        controller.abort();
