...
export function changeReport(
  ...
  return {
    ...
    changedFiles: [
      {
        path: "lib/example.ts",
        description: "Implemented the requested behavior.",
      },
    ],
    validation: [
      {
...
export async function submitChangeReport(
  request: AgentRunRequest,
  report: ChangeReport,
): Promise<string> {
  const tool = request.customTools?.find(
    (candidate) => candidate.name === "submit_change_report",
  );
  if (!tool) throw new Error("Request did not expose submit_change_report.");
  await tool.execute(
    "test-submit-change-report",
    report,
    undefined,
    undefined,
    {} as never,
