...
export function prDraft(overrides: Partial<PrDraft> = {}): PrDraft {
  return {
    ...
    simpleSummary:
      "This change fixes the reported behavior and is ready for review.",
    ...
    reviewInstructions: [
      "Review the behavior change and its regression coverage.",
    ],
...
export function issueDraft(
  ...
  return {
    ...
    simpleSummary:
      "This issue records a concrete reviewer finding for follow-up.",
...
export async function submitIssueDrafts(
  request: AgentRunRequest,
  collection: IssueDraftCollection,
): Promise<string> {
  const tool = request.customTools?.find(
    (candidate) => candidate.name === "submit_issue_drafts",
  );
  if (!tool) throw new Error("Request did not expose submit_issue_drafts.");
  await tool.execute(
    "test-submit-issue-drafts",
    collection,
    undefined,
    undefined,
    {} as never,
