...
export function revisionPlanResult(
  ...
  const defaults: RevisionPlanResult =
    status === "needs-human"
      ? {
          status,
          feedbackItems: [
...
export async function submitRevisionPlan(
  request: AgentRunRequest,
  result: RevisionPlanResult,
): Promise<string> {
  const tool = request.customTools?.find(
    (candidate) => candidate.name === "submit_revision_plan",
  );
  if (!tool)
    throw new Error(
      "Revision plan request did not expose submit_revision_plan.",
    );
  await tool.execute(
    "test-submit-revision-plan",
    result,
    undefined,
    undefined,
    {} as never,
