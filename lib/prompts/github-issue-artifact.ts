...
export function formatGitHubIssueArtifact(
  issue: GitHubIssue,
  ...
  const relationshipsSection = relationships
    ? `${formatIssueRelationships(relationships)}
  `
    : "";
...
function formatIssueRelationships(
  relationships: GitHubIssueRelationships,
): string {
  const summary = relationships.issueDependenciesSummary;
  const activeBlockers =
    summary?.blockedBy ??
    relationships.blockedBy.filter((issue) => issue.state !== "CLOSED").length;
  const totalBlockers =
    summary?.totalBlockedBy ?? relationships.blockedBy.length;
  const nativeAttrs = [
    `source="gh"`,
  ...
  if (relationships.repo)
    nativeAttrs.push(`repo="${escapePromptXmlAttribute(relationships.repo)}"`);
  ...
  if (relationships.unavailableReason) {
    lines.push(
      `    <native_dependencies_unavailable reason="${escapePromptXmlAttribute(relationships.unavailableReason)}" />`,
    );
  }

  if (relationships.blockedBy.length === 0)
    lines.push("    <blocked_by_none />");
  else {
    for (const issue of relationships.blockedBy) {
      lines.push(
        `    <blocked_by number="${issue.number}" state="${escapePromptXmlAttribute(issue.state)}" state_reason="${escapePromptXmlAttribute(issue.stateReason ?? "")}" closed_at="${escapePromptXmlAttribute(issue.closedAt ?? "")}" url="${escapePromptXmlAttribute(issue.url ?? "")}" title="${escapePromptXmlAttribute(issue.title)}" />`,
      );
    }
  }
  ...
  if (relationships.blocking.length === 0) lines.push("    <blocking_none />");
    ...
    for (const issue of relationships.blocking) {
      lines.push(
        `    <blocking number="${issue.number}" state="${escapePromptXmlAttribute(issue.state)}" state_reason="${escapePromptXmlAttribute(issue.stateReason ?? "")}" closed_at="${escapePromptXmlAttribute(issue.closedAt ?? "")}" url="${escapePromptXmlAttribute(issue.url ?? "")}" title="${escapePromptXmlAttribute(issue.title)}" />`,
      );
    }
  }
  ...
  if (relationships.bodyDeclaredBlockers.length === 0)
    lines.push("    <none />");
  else {
    for (const blocker of relationships.bodyDeclaredBlockers) {
      ...
      if (blocker.state)
        attrs.push(`state="${escapePromptXmlAttribute(blocker.state)}"`);
      if (blocker.stateReason !== undefined)
        attrs.push(
          `state_reason="${escapePromptXmlAttribute(blocker.stateReason ?? "")}"`,
        );
      if (blocker.closed !== undefined)
        attrs.push(`closed="${String(blocker.closed)}"`);
      if (blocker.closedAt !== undefined)
        attrs.push(
          `closed_at="${escapePromptXmlAttribute(blocker.closedAt ?? "")}"`,
        );
      if (blocker.url)
        attrs.push(`url="${escapePromptXmlAttribute(blocker.url)}"`);
      if (blocker.title)
        attrs.push(`title="${escapePromptXmlAttribute(blocker.title)}"`);
      if (blocker.unavailableReason)
        attrs.push(
          `unavailable_reason="${escapePromptXmlAttribute(blocker.unavailableReason)}"`,
        );
