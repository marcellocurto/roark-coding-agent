...
describe("reviewWithDependencies", () => {
  ...
  test("posts each reviewer's exact Markdown as its own comment", async () => {
    ...
    expect(
      await readFile(
        path.join(result.context.reviewDir, "review-a.md"),
        "utf8",
      ),
    ).toBe(
      "## Review A: Spec and Correctness\n\n**Changes requested.**\n\nFix malformed IDs.\n",
    ...
    expect(
      Bun.file(path.join(result.context.reviewDir, "review-a.json")).exists(),
    ).resolves.toBe(false);
    expect(
      Bun.file(path.join(result.context.reviewDir, "summary.json")).exists(),
    ).resolves.toBe(false);
  ...
  test("runs both reviewers read-only against the pinned diff and cleans mirrored artifacts", async () => {
    ...
    expect(agentCalls.every((call) => !call.fileEditingToolsEnabled)).toBe(
      true,
    );
    expect(agentCalls.every((call) => call.customTools === undefined)).toBe(
      true,
    );
    expect(
      agentCalls.every((call) =>
        call.prompt.includes(`git diff merge123..${feedback.pr.headRefOid} --`),
      ),
    ).toBe(true);
    expect(
      agentCalls.every((call) =>
        call.prompt.includes("Return only the final Markdown review"),
      ),
    ).toBe(true);
    expect(
      Bun.file(path.join(agent, ".roark/runs/pr/12/review-1")).exists(),
    ).resolves.toBe(false);
    expect(
      Bun.file(path.join(agent, ".git/roark/pr-review/12/review-1")).exists(),
    ).resolves.toBe(false);
    await rm(control, { recursive: true, force: true });
    await rm(agent, { recursive: true, force: true });
  });
  test("filters generated and unrelated feedback from reviewer context", async () => {
    ...
    feedback.comments = [
      ...
      {
        author: "roark",
        body: "<!-- roark:pr=12 phase=pr-review reviewer=a -->\nStale generated review",
      },
      {
        author: "roark",
    ...
    feedback.closingIssues = [
      {
        ...
        body: "Extract the contract",
        state: "OPEN",
        repository: "owner/repo",
        comments: [
          { author: "maintainer", body: "Preserve backward compatibility" },
        ],
      },
      {
        number: 127,
        title: "Pinned workspace",
        body: "Prepare the workspace",
        state: "OPEN",
        repository: "owner/repo",
      },
      {
        number: 9,
        title: "Unrelated external issue",
        body: "Ignore this",
        state: "OPEN",
    ...
    expect(reviewContext).toContain(
      "[resolved, outdated] lib/old.ts:10 reviewer: Historical concern",
    );
    expect(reviewContext).toContain(
      "[unresolved, current] lib/current.ts:20 reviewer: Active concern",
    );
  ...
  test("retains full verification diagnostics outside the truncated reviewer artifact", async () => {
    ...
    const fullVerification = await readFile(
      path.join(result.context.reviewDir, "verification-full.md"),
      "utf8",
    );
  ...
  test("retains a review when PR requirement text changes and does not publish it", async () => {
    ...
    const agent = await mkdtemp(
      path.join(tmpdir(), "roark-pr-review-stale-agent-"),
    );
    await initAgentRepo(agent);
    ...
    expect(
      Bun.file(path.join(result.context.reviewDir, "review-a.md")).exists(),
    ).resolves.toBe(true);
    expect(
      await readFile(
        path.join(result.context.reviewDir, "metadata.json"),
        "utf8",
      ),
  ...
  test("preserves completed artifacts when comment publishing fails", async () => {
    ...
    const agent = await mkdtemp(
      path.join(tmpdir(), "roark-pr-review-publish-agent-"),
    );
    await initAgentRepo(agent);
    const feedback = reviewFeedback();
    ...
    expect(
      Bun.file(path.join(reviewDir, "review-a.md")).exists(),
    ).resolves.toBe(true);
    expect(
      await readFile(path.join(reviewDir, "metadata.json"), "utf8"),
  ...
  test("waits for both reviewers before cleaning up after one reviewer fails", async () => {
    ...
    const agent = await mkdtemp(
      path.join(tmpdir(), "roark-pr-review-failure-agent-"),
    );
    await initAgentRepo(agent);
    const feedback = reviewFeedback();
      ...
      reviewWithDependencies(
        ...
        },
        {
          ...
          agentRunner: Effect.fnUntraced(function* (request) {
            ...
            yield* Effect.tryPromise({
              try: () => new Promise((resolve) => setTimeout(resolve, 20)),
              catch: (error) => error,
            });
    ...
    expect(
      Bun.file(
        path.join(control, ".roark/runs/pr/12/review-1/review-b.md"),
      ).exists(),
    ).resolves.toBe(true);
    expect(
      Bun.file(path.join(agent, ".git/roark/pr-review/12/review-1")).exists(),
