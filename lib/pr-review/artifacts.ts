...
export async function createPrReviewContext(
  options: ReviewPrCliOptions & { repo: string; agentCwd: string },
  application?: ApplicationExecution,
): Promise<PrReviewContext> {
  if (!application)
    return runApplicationPromise(
      fromLegacyPromise((application) =>
        createPrReviewContext(options, application),
      ),
      application,
    );
...
export async function writePrReviewArtifact(
  context: PrReviewContext,
  filename: string,
  content: string,
): Promise<void> {
...
export async function writePrReviewJson(
  context: PrReviewContext,
  filename: string,
  value: unknown,
): Promise<void> {
...
export async function writePrReviewInputArtifact(
  context: PrReviewContext,
  filename: string,
  content: string,
): Promise<void> {
...
export async function writePrReviewInputJson(
  context: PrReviewContext,
  filename: string,
  value: unknown,
): Promise<void> {
...
export async function removeAgentPrReviewArtifacts(
  context: PrReviewContext,
): Promise<void> {
