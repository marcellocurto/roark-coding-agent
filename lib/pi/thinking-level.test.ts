...
describe("thinking level resolution", () => {
  test.each([...thinkingLevels])("serializes Astra %s to a supported provider effort", async (requested) => {
    const astra = model("openai-codex", "gpt-6-astra");
    const resolution = resolveThinkingLevel(astra, requested);
    if (resolution.effective === "off") throw new Error("Astra requires reasoning");
    let payload: unknown;
    const tokenPayload = Buffer.from(JSON.stringify({
      "https://api.openai.com/auth": { chatgpt_account_id: "test-account" },
    })).toString("base64url");
    const response = await streamSimple(astra, {
      messages: [{ role: "user", content: "Inspect the change.", timestamp: 0 }],
    }, {
    ...
    expect(payload).toMatchObject({
      model: "gpt-6-astra",
      reasoning: { effort: requested === "off" || requested === "minimal" ? "low" : requested },
    });
  ...
  test("keeps max for a model that supports it", () => {
    expect(resolveThinkingLevel(model("openai-codex", "gpt-6-astra"), "max")).toMatchObject({
      requested: "max",
      effective: "max",
      clamped: false,
    });
