...
export class AgentOutputCollector {
  ...
  constructor(
    ...
    private readonly presentation: Presenter,
  ...
  event(event: AgentPresentationEvent): CompletedToolRun | undefined {
    ...
    const completed = this.tools.complete(
      event.toolCallId,
      event.endedAt ?? this.now(),
    );
    this.presentation.activity(
      this.display,
      summarizeToolCall(event.toolName, completed?.args, this.pathRoots),
      {
  ...
  finish(collectedResponse?: string): string {
    const response =
      collectedResponse === undefined || collectedResponse === ""
        ? this.streamedText.trim()
        : collectedResponse;
