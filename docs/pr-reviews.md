...
4. Resolve verification from `--verify`, repository config, safe package-script inference, or not-configured; run it once when present.
...
Use `--no-comment` for a fully local review. A changed PR head prevents publication but preserves the stale generation. Comment publishing failures also preserve completed artifacts and return an operational error.
