...
`review-pr` is the non-mutating review pass for an agent-authored PR. It reviews the complete contribution independently of existing comments, runs correctness and maintainability reviewers independently, and never edits, commits, pushes, or invokes `revise-pr`.
...
4. Resolve verification from `--verify`, trusted control-checkout configuration, or the shared default and run it once using the configured workspace and lifecycle setup.
