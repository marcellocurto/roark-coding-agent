...
import { runProcessOrThrowPromise } from "../lib/cli/process.ts";
...
try {
  ...
  await runProcessOrThrowPromise(["npm", "pack", "--pack-destination", temporaryRoot], { cwd: projectRoot });
  ...
  const globalRoot = (await runProcessOrThrowPromise(["npm", "root", "--global", "--prefix", prefix], {
    cwd: temporaryRoot,
  })).trim();
  ...
  await mkdir(target);
  await runProcessOrThrowPromise(["git", "init", "--quiet", target], { cwd: temporaryRoot });

  console.log("Checking the installed CLI outside the source checkout...");
  assert.equal((await runProcessOrThrowPromise([executable, "--version"], { cwd: target })).trim(), packageJson.version);
  assert.match(await runProcessOrThrowPromise([executable, "--help"], { cwd: target }), /roark <command>/);
  assert.equal((await runProcessOrThrowPromise([
  ...
  await cp(path.join(projectRoot, "scripts", "package-checks"), installedChecks, { recursive: true });
  for (const [fixture, description] of [
    ...
    ["verification.ts", "Effect verification and artifacts in a noninteractive target"],
  ] as const) {
    console.log(`Checking installed ${description}...`);
    await runProcessOrThrowPromise([process.execPath, path.join(installedChecks, fixture)], { cwd: target });
  }
